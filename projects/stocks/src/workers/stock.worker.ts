...
import { getDaasAddress, getFaasAddress, SecurityUtil } from "smartstock-core";
...
export class StockWorker {
  ...
  private csvToJSON(csv: string): StockModel[] {
    ...
    for (const line of csvContents) {
      ...
      if (
        lineParts &&
        Array.isArray(lineParts) &&
        lineParts.length === headerParts.length
      ) {
  ...
  async prepareStockForImportFromCsv(csv: string): Promise<StockModel[]> {
    ...
    return this.csvToJSON(csv).map((x) => {
      ...
      if (x && x.hasOwnProperty("downloadable")) {
        x.downloadable =
          x.downloadable.toString().toLowerCase().trim() === "true";
      } else {
      ...
      if (x && x.hasOwnProperty("purchasable")) {
        x.purchasable =
          x.purchasable.toString().toLowerCase().trim() === "true";
      } else {
        x.purchasable = true;
      }
    ...
    for (const stock of stocks) {
      ...
      for (const column of columns) {
        csv = csv
          .concat(
            stock[column]
              ? stock[column].toString().replace(new RegExp("[,-]", "ig"), "")
              : ""
          )
          .concat(", ");
      }
  ...
  sort(stocks: StockModel[]): StockModel[] {
    stocks
      ...
      .sort((a, b) => {
        ...
        if (
          a.product.toString().toLowerCase() <
          b.product.toString().toLowerCase()
        ) {
          return -1;
        }
        if (

...
@Component({
  ...
  template: `
    <app-layout-sidenav
      ...
      <ng-template #right>
        <app-transfer-cart *ngIf="headerData" [header]="headerData"></app-transfer-cart>
      </ng-template>
      <ng-template #body>
        <app-transfer-create-desktop-ux
          *ngIf="headerData"
          [header]="headerData"></app-transfer-create-desktop-ux>
      </ng-template>
    </app-layout-sidenav>
  `
})
...
export class TransferCreatePage implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    firstValueFrom(this.activateRoute.queryParams).then(q => {

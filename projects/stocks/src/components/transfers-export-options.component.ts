...
import { MessageService, PrintService } from "smartstock-core";
...
@Component({
  ...
  template: `
    <div style="display: flex; flex-direction: row">
      ...
      <button (click)="bottomSheetRef.dismiss()" mat-button color="warn">
        Close
      </button>
    </div>
...
export class TransfersExportOptionsComponent {
  ...
  constructor(
    ...
    private readonly data: { transfer: TransferModel }
  ) {}
  ...
  async printTransfer(): Promise<void> {
    ...
    for (let i = 0; i < this.data.transfer.items.length; i++) {
      items += `
------------------------------------
ITEM ${i + 1} : ${TransfersExportOptionsComponent.getProduct(
        this.data.transfer.items[i].product
      )}
QUANTITY : ${Intl.NumberFormat("en-US", { maximumFractionDigits: 2 }).format(
        this.data.transfer.items[i].quantity
      )}, PURCHASE PRICE : ${Intl.NumberFormat("en-US", {
        maximumFractionDigits: 2
      }).format(
        TransfersExportOptionsComponent.getPurchase(this.data.transfer.items[i])
      )}
`;
    }

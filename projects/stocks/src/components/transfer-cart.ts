    ...
    <div id="cart_view" [ngClass]="(deviceState.isSmallScreen | async)===true?'cart-mobile':'cart'">
          ...
          <div *ngFor="let cart of cartState.carts | async; let i=index">
            <mat-list-item matTooltip="{{cart.product}}">
              ...
              <mat-card-subtitle matLine>
                {{cart.quantity | number}}  @ {{cart.from_purchase | number}}
      ...
      <div
        ...
        <div class="checkout-container">
          <button [disabled]="(transferState.addTransferProgress | async)===true"
                  (click)="checkout()"
            ...
            <mat-progress-spinner *ngIf="(transferState.addTransferProgress | async)===true"
                                  mode="indeterminate"
                                  diameter="25"
                                  style="display: inline-block; float: right">
            </mat-progress-spinner>
            <span style="float: right" *ngIf="(transferState.addTransferProgress | async)===false">Record</span>
          </button>
        </div>
      </div>
    </div>
